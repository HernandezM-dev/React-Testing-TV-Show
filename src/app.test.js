...
test('Renders img,summary,dropdown/when option in drop down is selected episode cards are rendered', async ()=>{
    mockFetchShow.mockResolvedValueOnce(showData)
    render(<App/>)

    // const dropdown = await screen.findByText('Select a season')
    const dropdown = await screen.findByText('Select a season')
    fireEvent.mouseDown(dropdown)
    
    const season = await screen.getByRole('option', {name: /Season 1/i})
